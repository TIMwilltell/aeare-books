...
export async function updateBook(id: string, data: Partial<Omit<Book, 'id'>>): Promise<string> {
	const client = getClient();
	...
	if (data.readDate !== undefined) {
		updates.readDate = isDate(data.readDate) ? data.readDate.getTime() : (data.readDate ?? undefined);
	}
	if (data.quizDate !== undefined) {
		updates.quizDate = isDate(data.quizDate) ? data.quizDate.getTime() : (data.quizDate ?? undefined);
	}
	...
	await client.mutation(api.books.updateBook, { id: id as Id<'books'>, ...updates });
	return id;
}
...
export async function getProgressEventsByBook(bookId: string): Promise<ProgressEvent[]> {
	const client = getClient();
	const events = await client.query(api.progress.getByBook, { bookId: bookId as Id<'books'> });
	return (events ?? []).map(toProgressEvent);
}

export async function deleteProgressEventsByBook(_bookId: string): Promise<void> {
	...
	console.warn('deleteProgressEventsByBook: Not fully implemented - needs Convex mutation');
}

// AR Cache - Keep in memory for API caching (not core data)
const arCache = new Map<string, { arLevel: number; arPoints: number; cachedAt: number }>();
